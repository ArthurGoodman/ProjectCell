...
Console &Console::operator <<(QString str)
{
    output(str);
    return *this;
}
...
void Console::keyPressEvent(QKeyEvent *e)
{
    ...
    if((e->key() == Qt::Key_V && e->modifiers() == Qt::ControlModifier)) QTextEdit::keyPressEvent(e);
    ...
    {
    case Qt::Key_Return: onReturn(); break;

    case Qt::Key_Up: historyBack(); break;
    case Qt::Key_Down: historyForward(); break;

    case Qt::Key_Backspace: if(textCursor().positionInBlock() > prompt.length())
    ...
    case Qt::Key_Right: if(e->modifiers() == Qt::NoModifier || e->modifiers() == Qt::ControlModifier)
            QTextEdit::keyPressEvent(e); break;
...
void Console::historyForward()
{
    ...
    if(historyPos == history->length() - 1) cursor.insertText(prompt);
    else cursor.insertText(prompt + history->at(historyPos + 1));
